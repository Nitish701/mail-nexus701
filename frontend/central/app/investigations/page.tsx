...
const navItems = [
  { label: 'SOC Overview', href: '/' }, { label: 'Live Feed', href: '/live-feed' },
  { label: 'Campaigns', href: '/campaigns' }, { label: 'Investigations', href: '/investigations' },
  { label: 'Threat Intelligence', href: '/threat-intelligence' }, { label: 'Reports', href: '/reports' }
];

export default async function CentralInvestigationsPage() {
  const { reports } = await fetchCentralReports();
  return <AppShell title="Central investigations" subtitle="Suspicious activity only" navItems={navItems}>
